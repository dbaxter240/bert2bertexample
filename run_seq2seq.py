...
def train(args, train_dataset, model, tokenizer, pad_token_label_id):
    ...
    if os.path.exists(args.encoder_model_name_or_path):
        ...
        if args.encoder_model_name_or_path.split("-")[-1].split("/")[0].isdigit():
            global_step = int(args.encoder_model_name_or_path.split("-")[-1].split("/")[0])
        else:
            global_step = 0
...
def load_and_cache_examples(args, tokenizer, pad_token_label_id, mode):
    ...
    cached_features_file = os.path.join(
        args.data_dir,
        "cached_{}_{}_{}".format(
            mode, list(filter(None, args.encoder_model_name_or_path.split("/"))).pop(), str(args.max_seq_length),
        ),
    )
    ...
    all_decoder_ids = torch.tensor([feat.decoder_ids for feat in features], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_output_ids, all_input_mask, all_output_mask, all_segment_ids, all_decoder_ids)
    return dataset
...
def main():
    ...
    parser.add_argument(
        "--encoder_model_type", default=None, type=str, required=True, help="Model type selected",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--decoder_model_type", default=None, type=str, required=True, help="Model type selected",
    )
    parser.add_argument(
