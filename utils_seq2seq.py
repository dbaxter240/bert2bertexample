...
class InputFeatures(object):
    ...
    def __init__(
        self, input_ids, output_ids, input_mask, output_mask, segment_ids, formatted_tokens, decoder_ids,
    ):
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        special_tokens_count = 3 if encoder_model_type in ["roberta"] else (2 if encoder_model_type != "gpt2" else 0)
        ...
        if encoder_model_type not in ["gpt2"]:
            tokens += [sep_token]
            formatted_tokens += [pad_token_label_id]

        if encoder_model_type in ["roberta"]:
        ...
        if encoder_model_type in ["xlnet"]:
            tokens += [cls_token]
            formatted_tokens += [pad_token_label_id]
            segment_ids += [cls_token_segment_id]
        # gpt2 has no cls_token
        elif encoder_model_type not in ["gpt2"]:
            pass
        ...
        decoder_ids = [pad_token] + output_ids[:len(input_ids)-1]
        ...
        assert len(decoder_ids) == max_seq_length
        ...
        if ex_index < 5:
            ...
            logger.info("decoder_ids: %s", " ".join([str(x) for x in decoder_ids]))
